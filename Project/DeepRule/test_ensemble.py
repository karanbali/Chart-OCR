...
def Pre_load_nets( id_cuda, data_dir, cache_dir):
    methods = {}
    ...
    db_bar, nnet_bar = load_net(50000, "CornerNetPureBar", data_dir+'/bardata', data_dir+'/bardata',
                                id_cuda)
                                
    db_bar1, nnet_bar1 = load_net(150000, "CornerNetPureBar", data_dir+'/bardata', data_dir+'/bardata',
                                id_cuda)
                                
    db_bar2, nnet_bar2 = load_net(250000, "CornerNetPureBar", data_dir+'/bardata', data_dir+'/bardata',
                                id_cuda)
                                
    db_bar3, nnet_bar3 = load_net(350000, "CornerNetPureBar", data_dir+'/bardata', data_dir+'/bardata',
                                id_cuda)
        
    db_bar4, nnet_bar4 = load_net(450000, "CornerNetPureBar", data_dir+'/bardata', data_dir+'/bardata',
                                id_cuda)
                                
    
    path = 'testfile.test_%s' % "CornerNetPureBar"
    testing_bar = importlib.import_module(path).testing
    methods['Bar'] = [db_bar, nnet_bar, testing_bar, nnet_bar1, nnet_bar2, nnet_bar3, nnet_bar4]
    
    

    #if type == "Pie":
    
    db_pie, nnet_pie = load_net(50000, "CornerNetPurePie", data_dir+'/piedata', data_dir+'/piedata',
                                id_cuda)
    ...
    db_line, nnet_line = load_net(50000, "CornerNetLine", data_dir+'/linedata', data_dir+'/linedata',
                                  id_cuda)
                                  
    db_line1, nnet_line1 = load_net(150000, "CornerNetLine", data_dir+'/linedata', data_dir+'/linedata',
                                  id_cuda)
                                  
    db_line2, nnet_line2 = load_net(250000, "CornerNetLine", data_dir+'/linedata', data_dir+'/linedata',
                                  id_cuda)
                                  
    db_line3, nnet_line3 = load_net(350000, "CornerNetLine", data_dir+'/linedata', data_dir+'/linedata',
                                  id_cuda)
                                  
    db_line4, nnet_line4 = load_net(450000, "CornerNetLine", data_dir+'/linedata', data_dir+'/linedata',
                                  id_cuda)
                                  
                                  
    path = 'testfile.test_%s' % "CornerNetLine"
    testing_line = importlib.import_module(path).testing
    methods['Line'] = [db_line, nnet_line, testing_line, nnet_line1, nnet_line2, nnet_line3, nnet_line4]
    
    
    
    db_line_cls, nnet_line_cls = load_net(20000, "CornerNetLineClsReal",  data_dir+'/linedata', data_dir+'/linedata',
                                           id_cuda)
...
def test(image_path, data_type=1, debug=False, suffix=None, min_value_official=None, max_value_official=None, ensemble_flag=0):
    ...
    with torch.no_grad():
        ...
        if data_type == 0:
            ...
            results = methods['Bar'][2](image, methods['Bar'][0], methods['Bar'][1], debug=False, nnet1=methods['Bar'][3], nnet2=methods['Bar'][4], nnet3=methods['Bar'][5], nnet4=methods['Bar'][6])
        ...
        if data_type== 1:
            ...
            results = methods['Line'][2](image, methods['Line'][0], methods['Line'][1], debug=False, nnet1=methods['Line'][3], nnet2=methods['Line'][4], nnet3=methods['Line'][5], nnet4=methods['Line'][6])
            keys = results[0]
            hybrids = results[1]
...
def parse_args():
    ...
    parser.add_argument("--base_dir", dest="base_dir", default="data/linedata(1028)", type=str)
    parser.add_argument('--cache_path', dest="cache_path", type=str)
    parser.add_argument('--e_flag', dest="e_flag", type=int, default=0)
...
if __name__ == "__main__":
    ...
    #base_dir = '/content/gdrive/MyDrive/Fall_21/Final_projects/'
    methods = Pre_load_nets(0, base_dir, args.cache_path)
    ...
    for image in tqdm(images):
        ...
        data = test(path,ensemble_flag=e_flag)
        #data = test(path,1)
        rs_dict[image] = data
