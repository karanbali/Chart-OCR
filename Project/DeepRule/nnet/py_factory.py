...
class NetworkFactory(object):
    ...
    def cuda(self, cuda_id=torch.device('cuda')):
        self.model.cuda(torch.device('cuda'))
        self.network.cuda(torch.device('cuda'))
        self.cuda_id = torch.device('cuda')
    ...
    def save_params_split(self, cache_file):
    ...
    def load_params_split(self, cache_file):
        cache_file = cache_file
        print("loading model from {}".format(cache_file))
        with open(cache_file, "rb") as f:
            if torch.cuda.is_available():
