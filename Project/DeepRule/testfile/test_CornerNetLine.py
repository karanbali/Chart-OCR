...
def kp_decode(nnet, images, K, ae_threshold=0.5, kernel=3):
    with torch.no_grad():
            ...
            #print("detections_tl_detections_br!!!!",detections_tl_detections_br)
...
def kp_detection(image, db, nnet, debug=False, cuda_id=torch.device('cuda'), decode_func=kp_decode, nnet1=None, nnet2=None, nnet3=None, nnet4=None):
    K = db.configs["top_k"]
    ...
    if True:
        ...
        if nnet1 != None and nnet2 != None and nnet3 != None and nnet4 != None:
            ...
            #print("det_Key",dets_key)
            detections_point_key.append(dets_key)
            detections_point_hybrid.append(dets_hybrid)
            
            
            dets_key1, dets_hybrid1, time_backbone1, time_psn1, flag1 = decode_func(nnet1, images, K, ae_threshold=ae_threshold, kernel=nms_kernel)
            time_backbones += time_backbone1
            time_psns += time_psn1
            #print(dets_key.shape)
            _rescale_points(dets_key1, ratios, borders, sizes)
            _rescale_points(dets_hybrid1, ratios, borders, sizes)
            #print("det_Key",dets_key)
            detections_point_key.append(dets_key1)
            detections_point_hybrid.append(dets_hybrid1)
            
            
            dets_key2, dets_hybrid2, time_backbone2, time_psn2, flag2 = decode_func(nnet2, images, K, ae_threshold=ae_threshold, kernel=nms_kernel)
            time_backbones += time_backbone2
            time_psns += time_psn2
            #print(dets_key.shape)
            _rescale_points(dets_key2, ratios, borders, sizes)
            _rescale_points(dets_hybrid2, ratios, borders, sizes)
            #print("det_Key",dets_key)
            detections_point_key.append(dets_key2)
            detections_point_hybrid.append(dets_hybrid2)
            
            dets_key3, dets_hybrid3, time_backbone3, time_psn3, flag3 = decode_func(nnet3, images, K, ae_threshold=ae_threshold, kernel=nms_kernel)
            time_backbones += time_backbone3
            time_psns += time_psn3
            #print(dets_key.shape)
            _rescale_points(dets_key3, ratios, borders, sizes)
            _rescale_points(dets_hybrid3, ratios, borders, sizes)
            #print("det_Key",dets_key)
            detections_point_key.append(dets_key3)
            detections_point_hybrid.append(dets_hybrid3)
            
            
            dets_key4, dets_hybrid4, time_backbone4, time_psn4, flag4 = decode_func(nnet4, images, K, ae_threshold=ae_threshold, kernel=nms_kernel)
            time_backbones += time_backbone4
            time_psns += time_psn4
            #print(dets_key.shape)
            _rescale_points(dets_key4, ratios, borders, sizes)
            _rescale_points(dets_hybrid4, ratios, borders, sizes)
            #print("det_Key",dets_key)
            detections_point_key.append(dets_key4)
            detections_point_hybrid.append(dets_hybrid4)
...
def testing(image,db, nnet, debug=False, nnet1=None, nnet2=None, nnet3=None, nnet4=None):
    cuda_id=torch.device('cuda')
    return globals()[system_configs.sampling_function](image,db, nnet, debug, cuda_id, nnet1=nnet1, nnet2=nnet2, nnet3=nnet3, nnet4=nnet4)
