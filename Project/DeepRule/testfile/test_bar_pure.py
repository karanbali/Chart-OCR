...
def kp_detection(db, nnet, result_dir, debug=False, decode_func=kp_decode):
    ...
    #result_json_path = point_json_tl
    #result_json_path = point_json_br
    result_json_path = '/content/gdrive/MyDrive/Fall_21/Final_projects/DeepRule/save/bar.json'
    gt_path = "/content/gdrive/MyDrive/Fall_21/Final_projects/bardata/bar/annotations/instancesBar(1031)_test2019.json"
    #with open(result_json_path, "r") as f:
    #    result_json = json.load(f)
    for cls_type in range(1, categories+1):
        rt = db.evaluate_new(result_json_path, [cls_type], image_ids)
        print("RTRTRT!!!",rt)
    """
    
    from RuleGroup.Bar import GroupBarRaw
    result_tl = point_json_tl
    tls = json.load(open(result_tl))
    result_br = point_json_br
    brs = json.load(open(result_br))
